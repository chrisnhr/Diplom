...
WITH
  ...
  cte_test_window AS(
    ...
    AND CALENDAR_DATE BETWEEN FIRST_ANSPRACHE_DATE AND LEAST(FIRST_ANSPRACHE_DATE+364, LAST_ANSPRACHE_DATE)
  ),
  cte_twin_window AS(
    SELECT *
    FROM cte_top_data
    WHERE TEST_ITEM_COMMUNICATIONKEY != TWIN_ITEM_COMMUNICATIONKEY
    AND CALENDAR_DATE BETWEEN FIRST_ANSPRACHE_DATE-364 AND FIRST_ANSPRACHE_DATE - 364 + DATE_DIFF(LEAST(FIRST_ANSPRACHE_DATE+364, LAST_ANSPRACHE_DATE),FIRST_ANSPRACHE_DATE, DAY)
  )

SELECT
  *,
  DATE_DIFF(LEAST(FIRST_ANSPRACHE_DATE+364, LAST_ANSPRACHE_DATE), FIRST_ANSPRACHE_DATE, DAY) AS TEST_PERIOD,
  DATE_DIFF(FIRST_ANSPRACHE_DATE - 364 + DATE_DIFF(LEAST(FIRST_ANSPRACHE_DATE+364, LAST_ANSPRACHE_DATE),FIRST_ANSPRACHE_DATE, DAY), FIRST_ANSPRACHE_DATE-364, DAY) TWIN_PERIOD
FROM
  cte_test_window
UNION ALL
SELECT *,
