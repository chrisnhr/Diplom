CREATE OR REPLACE TABLE `brain-flash-dev.dagster_common.CN_item_selections` AS
WITH
  ...
  ITEM_COMMUNICATIONKEY
  ORDER BY
    AVG_ANSPRACHE_UNBIASED_AVLBL_OTTO_DE
  LIMIT
    (50) ),
  cte_dates AS(
    SELECT 
  ...
  cte_twin_selection AS (
    ...
    FROM cte_test_selection test
    JOIN 
    `brain-flash-dev.dagster_attributes.twins_lwg_fashion` twin
    USING
      (ITEM_COMMUNICATIONKEY)
    JOIN
    ...
    ON
    twin.twin_item_communicationkey = test_dates.ITEM_COMMUNICATIONKEY
    WHERE
...
WHERE
  RANK <= 10
ORDER BY TEST_ITEM_COMMUNICATIONKEY, DISTANCE